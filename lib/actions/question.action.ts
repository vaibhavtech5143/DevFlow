...
export async function getQuestionById(params:GetQuestionByIdParams){
    try {
        ...
        const question = await Question.findById(questionId).populate({path:'tags',model:Tag}).populate({path:'author',model:User,select:'_id clerkId name picture'}).sort({createdAt:-1});
